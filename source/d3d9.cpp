...
#pragma region D3D9_WrapperFunctions
...
DWORD returnAddyResolutionOverride;
void __declspec(naked) ResolutionOverride()
{
	__asm
	{
		mov edx,[ebp-0x20]
		...
		mov eax,[scrHeight]
		mov [edx+0xC],eax
		jmp [returnAddyResolutionOverride]
	}
}
...
bool WINAPI DllMain(HMODULE hModule, DWORD fdwReason, LPVOID lpReserved) {
    ...
    case DLL_PROCESS_ATTACH:
    {
		...
		*(int*)((uintptr_t)BaseModule + 0x02B6F + 6) = hookSettings.scrWidth;
		*(int*)((uintptr_t)BaseModule + 0x02B79 + 6) = hookSettings.scrHeight;
		HookInsideFunction((uintptr_t)BaseModule + 0x0002720, ResolutionOverride, &returnAddyResolutionOverride, 0x6);
        break;
    }
